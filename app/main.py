...
async def lifespan(app: FastAPI):
    ...
    telemetry_svc = TelemetryService(
        ...
        rl_service=rl_svc,
        rgb_streamer=rgb_streamer,
        depth_streamer=depth_streamer,
        map_streamer=map_provider   # 注意 map_provider 需实现 get_fps/get_latency_ms
    )
    class Services: pass
...
@app.get("/api/map_meta")
async def get_map_meta():
    map_svc = app.state.services.map_service
    return map_svc.get_metadata()