...
# ==========================================
from core.ros2.bridges.gazebo import GazeboBridge
...
def fetch_tb3_urdf() -> str:
    ...
    bridge = GazeboBridge(node_name='urdf_prefetcher')
...
class TurtleBot3NavEnv(gym.Env):
    ...
    def __init__(self, robot_urdf: Optional[str] = None, env_config: Optional[Dict[str, Any]] = None):
        ...
        self.bridge = GazeboBridge(node_name=f'{self.robot["name"]}_gym_env')
...
#     """
#     在主进程中提前拉取完整的 URDF。
#     重构说明：直接复用 GazeboBridge 的单例节点和后台 Spin 线程，
